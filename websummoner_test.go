...
func TestSessionCreated(t *testing.T) {
	...
	sessions.Remove(sess.Value.SessionId)
	queue.Release()
}
...
func TestSessionCreatedW3C(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, resp.StatusCode, http.StatusOK)
	sessID := sessionIDFrom(t, resp.Body)
...
func TestClientDisconnected(t *testing.T) {
	...
	go func() { _, _ = http.DefaultClient.Do(req.WithContext(ctx)) }()
...
func TestProxySession(t *testing.T) {
	...
	resp, err = http.Get(With(srv.URL).Path(fmt.Sprintf("/wd/hub/session/%s/url", sessID)))
	assert.NoError(t, err)
	assert.Equal(t, resp.StatusCode, http.StatusOK)
...
func TestProxySessionPanicOnAbortHandler(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodGet, With(srv.URL).Path(fmt.Sprintf("/wd/hub/session/%s/url?abort-handler=true", sessID)), nil)
...
func TestSessionDeleted(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodDelete,
		With(srv.URL).Path(fmt.Sprintf("/wd/hub/session/%s", sessID)), nil)
	_, err = http.DefaultClient.Do(req)
	assert.NoError(t, err)
...
func TestSessionOnClose(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodDelete,
		With(srv.URL).Path(fmt.Sprintf("/wd/hub/session/%s/window", sessID)), nil)
...
func TestProxySessionCanceled(t *testing.T) {
	...
	_, ok := sessions.Get(sessID)
	assert.True(t, ok)

	req, _ := http.NewRequest(http.MethodGet, With(srv.URL).Path(fmt.Sprintf("/wd/hub/session/%s/url?timeout=1s", sessID)), nil)
...
func TestProxySessionTimeout(t *testing.T) {
	...
	_, ok = sessions.Get(sessID)
	assert.True(t, ok)
	_, _ = http.Get(With(srv.URL).Path(fmt.Sprintf("/wd/hub/session/%s/url", sessID)))
...
func TestFileUpload(t *testing.T) {
	...
	resp, err = http.Post(With(srv.URL).Path(fmt.Sprintf("/wd/hub/session/%s/file", sessID)), "", bytes.NewReader(fileContents))
	assert.NoError(t, err)
	assert.Equal(t, resp.StatusCode, http.StatusOK)
...
func TestFileUploadBadJson(t *testing.T) {
	...
	resp, err = http.Post(With(srv.URL).Path(fmt.Sprintf("/wd/hub/session/%s/file", sessID)), "", bytes.NewReader([]byte(`malformed json`)))
	assert.NoError(t, err)
...
func TestFileUploadNoFile(t *testing.T) {
	...
	resp, err = http.Post(With(srv.URL).Path(fmt.Sprintf("/wd/hub/session/%s/file", sessID)), "", bytes.NewReader([]byte(`{}`)))
	assert.NoError(t, err)
	assert.Equal(t, resp.StatusCode, http.StatusBadRequest)
...
func TestFileUploadTwoFiles(t *testing.T) {
	...
	resp, err = http.Post(With(srv.URL).Path(fmt.Sprintf("/wd/hub/session/%s/file", sessID)), "", bytes.NewReader([]byte(`{"file":""}`)))
	assert.NoError(t, err)
	assert.Equal(t, resp.StatusCode, http.StatusBadRequest)
...
func testFileDownload(t *testing.T, path func(string) string) {
	...
	rsp, err := http.Get(With(srv.URL).Path(path(sessID)))
	assert.NoError(t, err)
	assert.Equal(t, resp.StatusCode, http.StatusOK)
...
func testClipboard(t *testing.T, path func(string) string) {
	...
	_, err = http.Post(With(srv.URL).Path(path(sessID)), "text/plain", bytes.NewReader([]byte("any-data")))
	assert.NoError(t, err)
	assert.Equal(t, resp.StatusCode, http.StatusOK)
...
func TestDevtools(t *testing.T) {
	...
	u := fmt.Sprintf("ws://%s/devtools/%s", srv.Listener.Addr().String(), sessID)
...
// sessionIDFrom pulls the session id out of a create-session response, which
// carries it at the top level or under "value" depending on the driver dialect.
func sessionIDFrom(t *testing.T, r io.Reader) string {
	t.Helper()
	var body struct {
		SessionID string `json:"sessionId"`
		Value     struct {
	...
	assert.NoError(t, json.NewDecoder(r).Decode(&body))
