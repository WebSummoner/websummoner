...
| Video recording | H.264 **with audio on every image** (`enableVideo`, `enableAudio` opt-out) | H.264; audio best-effort, silent on some images, no opt-out | H.264 with audio (`enableAudio`, since 2.7.5) + **HAR capture** | ffmpeg **sidecar container per browser container**, `se:recordVideo`, failure-only retention, RCLONE upload; **not supported for headless** |
